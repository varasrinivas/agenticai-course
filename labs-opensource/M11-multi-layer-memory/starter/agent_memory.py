"""
M11 Lab - Step 3: AgentMemory — the facade over both layers
============================================================
One build_context() call assembles everything the agent should see.
...
class AgentMemory:
    ...
    def build_context(self, query: str) -> list[dict]:
        ...
        TODO — assembly order IS the lesson:
        1. vector_hits = self.vector.recall(query, k=self.vector_recall_k)
           Keep only hits with score >= 0.5 (filter low-confidence noise).
           If any survive, build ONE system message:
             "[RELEVANT PAST FACTS]\\n- [0.87] fact one\\n- [0.61] fact two"
           GOTCHA: inject as a SYSTEM message — a fake user/assistant turn
           confuses instruction-tuned models.
        2. Then extend with self.buffer.get() — the buffer comes LAST so the
           most recent conversation always wins over older context.
        3. Return the list.
        """
    ...
    def save_turn(self, user_msg: str, assistant_msg: str) -> None:
        ...
        TODO:
        1. self.buffer.add("user", user_msg); self.buffer.add("assistant", assistant_msg)
        2. Heuristic: if either message is longer than 80 chars, it probably
           contains facts worth keeping — save a condensed line to vector memory:
           self.vector.save(f"User asked: {user_msg[:120]} -> Agent: {assistant_msg[:200]}")
        """
        pass  # Remove this line when you add your code


def chat(memory: AgentMemory, user_msg: str) -> str:
    """(COMPLETE) One agent turn using the assembled context."""
...
if __name__ == "__main__":
    ...
    # Simulate a process restart: NEW AgentMemory → empty buffer,
    # but the SAME vector persist dir → facts survive.
