"""
CAPSTONE C3: ReAct Entity Resolution Agent
===========================================
System prompt, tool schemas, and handlers are provided.
You build run_entity_agent() — the loop that puts 30 modules together.
...
def run_entity_agent(query: str, verbose: bool = True) -> str:
    ...
    2. Loop AT MOST 15 times (resolution legitimately needs 8-12 tool calls):
       a. response = client.chat.completions.create(model=MODEL,
              messages=messages, tools=TOOLS)
          (try/except → return f"Error calling model: {e}")
