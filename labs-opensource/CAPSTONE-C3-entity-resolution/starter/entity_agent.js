/**
 * CAPSTONE C3: ReAct Entity Resolution Agent (Node.js)
 * =====================================================
 * System prompt, tool schemas, and handlers are provided.
 * You build runEntityAgent() — the loop that puts 30 modules together.
...
const MODEL = "mistral"; // swap to "mixtral" or "llama3" for stronger reasoning
...
/**
 * The ReAct loop — your capstone.
 *
 * TODO:
 * 1. const messages = [{ role: "system", content: SYSTEM_PROMPT },
 *                      { role: "user", content: query }];
 * 2. Loop AT MOST 15 times (resolution legitimately needs 8-12 tool calls):
 *    a. response = await client.chat.completions.create({ model: MODEL,
 *         messages, tools: TOOLS })  (try/catch → `Error calling model: ...`)
 ...
 *           tool_calls: toolCalls.map((tc) => ({ id: tc.id, type: "function",
 *             function: { name: tc.function.name, arguments: tc.function.arguments } })) }
 *       - For each tc:
 *         * if verbose: log `  [tool] ${tc.function.name}(${tc.function.arguments.slice(0, 80)})`
 *         * const handler = HANDLERS[tc.function.name];
 *         * let args; try { args = JSON.parse(tc.function.arguments); }
 *           catch { args = {}; }   ← malformed args ≠ crash
 ...
 * 3. After the loop: return "Resolution exceeded maximum iterations (15).";
 */
