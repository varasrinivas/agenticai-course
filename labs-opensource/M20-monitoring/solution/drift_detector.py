"""
M20 Lab - Step 1: Z-Score Drift Detector — SOLUTION
====================================================
...
class RollingWindow:
    """Fixed-size FIFO of float samples."""
...
class DriftDetector:
    ...
    def __init__(self, window_size: int = 2016):
    ...
    def check_drift(self, metric: str, window: RollingWindow) -> Optional[DriftAlert]:
        ...
        if window.size < self.MIN_SAMPLES:
            return None  # small samples scream false alarms

        current = window.values[-1]
        history = window.values[:-1]
        m = mean(history)
        s = stddev(history)
        if s == 0:
            return None  # constant history — skip
...
if __name__ == "__main__":
    ...
    fired = []
    for i in range(5):
        alerts = det.record(quality_score=0.55, tokens_used=2800, tool_calls_made=5)
        for a in alerts:
            fired.append(a)
