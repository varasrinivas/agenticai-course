...
=====================================================================
Three tools: get_weather, calculate, get_time. Imported by tool_agent.py.
Run standalone to sanity-check: python tools.py
"""
...
def run_tool(name: str, args: dict) -> str:
    ...
    NOTE: errors are returned as DATA, never raised — the model can read
...
if __name__ == "__main__":
    ...
    print(f"Defined {len(TOOLS)} tools: {', '.join(names)}")
    print(f"Test: {run_tool('get_weather', {'city': 'Tokyo'})}")
    print(f"Test: {run_tool('calculate', {'expression': '(15 * 7) + 23'})}")
    print(f"Test: {run_tool('get_weather', {'city': 'Atlantis'})}")
