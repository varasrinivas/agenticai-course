/**
 * M15 Lab - Step 1: SubprocessExecutor (Node.js)
 * ===============================================
 * Run model-written Python in a child process with a timeout.
 * Run: node sandbox_executor.js    (smoke tests, no LLM needed)
 */
...
export class SubprocessExecutor {
  ...
  /**
   * Execute code, return { stdout, stderr, exitCode }. Must never throw.
   *
   * TODO:
   * 1. const timeout = (timeoutSeconds ?? this.timeoutSeconds) * 1000;
   * 2. Write `code` to a temp file: join(tmpdir(), `sandbox_${Date.now()}.py`)
   * 3. try:
   *      const { stdout, stderr } = await execFileAsync(PYTHON, [tmpFile],
   *        { timeout, encoding: "utf8" });   ← execFile = no shell = no injection
   *      return { stdout, stderr, exitCode: 0 };
   *    catch (err):
   *      if (err.killed || err.signal === "SIGTERM")
   *        return { stdout: "", stderr: `Execution timed out after ...s.`,
