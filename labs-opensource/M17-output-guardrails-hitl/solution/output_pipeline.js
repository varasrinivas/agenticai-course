/**
 * M17 Lab: Output Guardrails & HITL — SOLUTION (Node.js)
 * =======================================================
...
async function checkHallucination(responseText, sourceDocs) {
  ...
  try {
    const result = await client.chat.completions.create({
      model: "mistral",
      messages: [{ role: "user", content: HALLUCINATION_PROMPT(sources, responseText) }],
    });
    const raw = (result.choices[0].message.content ?? "").replace(/```json|```/g, "").trim();
    const parsed = JSON.parse(raw);

    const known = new Set(["supported", "unsupported", "contradicted"]);
    const claims = (parsed.claims ?? []).map((c) => ({
      ...c,
      status: known.has(c.status) ? c.status : "unsupported",
    }));
    ...
    if (contradicted.length) {
      return { result: "block", reason: `${contradicted.length} contradicted claim(s)`, claims };
    }
    if (unsupported.length >= 2) {
      return { result: "flag", reason: `${unsupported.length} unsupported claim(s)`, claims };
    }
    return { result: "pass", claims };
  } catch (e) {
    // Quality gates degrade to HUMAN REVIEW, not silently to pass
    return { result: "flag", reason: `Check failed: ${e.message}`, claims: [] };
  }
}

const INPUT_PRICE = 2.0 / 1_000_000;
const OUTPUT_PRICE = 6.0 / 1_000_000;
...
class CostTracker {
  ...
  canAfford(estimatedInput = 5000, estimatedOutput = 1000) {
    const estimatedCost = estimatedInput * INPUT_PRICE + estimatedOutput * OUTPUT_PRICE;
...
class CircuitBreaker {
  ...
  canExecute() {
    ...
    if (this.state === "open") {
      if ((Date.now() - this.openedAt) / 1000 >= this.cooldownSeconds) {
        this.state = "half_open";
        return true; // exactly one test request
      }
      return false;
    }
    return false; // half_open: a test is already in flight
  }
  ...
  recordFailure() {
    ...
    if (this.state === "half_open") {
      ...
      this.cooldownSeconds *= 2; // exponential backoff on failed test
