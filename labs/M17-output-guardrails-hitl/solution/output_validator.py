...
def check_json_structure(output: dict, required_fields: list[str]) -> dict:
    ...
    missing = sorted(required_set - output_keys)
    extra = sorted(output_keys - required_set)
...
def check_hallucination_markers(text: str) -> dict:
    ...
    markers_found = [marker for marker in HALLUCINATION_MARKERS if marker in text_lower]
    penalty = min(len(markers_found) * 0.1, 0.5)
...
def check_pii_in_output(text: str) -> dict:
    ...
    for pii_name, pattern, replacement in PII_PATTERNS:
        if re.findall(pattern, redacted):
...
def validate_output(output: dict, expected_fields: list[str]) -> dict:
    ...
    """
    structure_check = check_json_structure(output, expected_fields)
    ...
    if "response" in output and isinstance(output["response"], str):
        hallucination_check = check_hallucination_markers(output["response"])
        pii_check = check_pii_in_output(output["response"])
        if pii_check["has_pii"]:
            result_output["response"] = pii_check["redacted_text"]
