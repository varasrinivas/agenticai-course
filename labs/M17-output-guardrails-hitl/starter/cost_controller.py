...
class CostController:
    ...
    def track_usage(self, input_tokens: int, output_tokens: int) -> dict:
        ...
        Returns:
            {
                "call_cost": float,        # cost of this single call
                "cumulative_cost": float,   # total cost so far
                "budget_remaining": float,  # dollars left
                "budget_exceeded": bool     # True if over limit
            }
        """
        # TODO 2: Calculate the cost of this call using COST_PER_INPUT_TOKEN
        # and COST_PER_OUTPUT_TOKEN. Update current_cost, call_count,
        # total_input_tokens, total_output_tokens.
        call_cost = 0.0

        return {
            "call_cost": round(call_cost, 6),
            "cumulative_cost": round(0.0, 6),
            "budget_remaining": round(self.budget_limit, 6),
        ...
        Returns:
            {
                ...
                "would_exceed": bool,
                "budget_after": float  # projected remaining budget after call
            }
        """
        # TODO 4: Calculate the estimated cost and check if current_cost + estimated
        # would exceed budget_limit.
        estimated_cost = 0.0

        return {
            "estimated_cost": round(estimated_cost, 6),
            "would_exceed": False,
            "budget_after": round(self.budget_limit, 6),
        }

    def reset(self):
        """Reset all counters for a new request."""
        # TODO 5: Reset current_cost, call_count, total_input_tokens,
        # total_output_tokens to zero.
        pass

    def get_summary(self) -> dict:
        """Return a summary of all usage tracking."""
        return {
            "total_cost": round(getattr(self, "current_cost", 0.0), 6),
            "budget_limit": self.budget_limit,
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("Self-test complete. Fill in TODOs to see correct cost tracking.")
    print("=" * 60)
