...
async function transformQueryHyde(query) {
  ...
  try {
    ...
    const hydeText = response.content[0].text;
    observeStep("HyDE", `Hypothetical answer: ${hydeText.slice(0, 150)}...`);
    return hydeText;
  } catch (e) {
    observeStep("HyDE", `Error: ${e.message}. Falling back to original query.`);
    return query;
  }
}
...
async function transformQueryMulti(query) {
  ...
  try {
    ...
    observeStep("MULTI-QUERY", `Generated ${queries.length} alternative queries`);
    queries.forEach((q, i) => observeStep("MULTI-QUERY", `  ${i + 1}. ${q}`));
    return queries;
  } catch (e) {
    observeStep(
      "MULTI-QUERY",
      `Error: ${e.message}. Falling back to original query.`
    );
    return [query];
  }
}
...
async function advancedRagPipeline(query, collection, bm25Index) {
  ...
  // Step 3: Deduplicate by doc ID (keep highest score)
  ...
  const deduped = [...bestById.values()].sort((a, b) => b.score - a.score);
  observeStep("PIPELINE", `Deduplicated to ${deduped.length} unique documents`);

  // Step 4: Re-rank top candidates with Claude
  const topCandidates = await rerankWithClaude(query, deduped.slice(0, 7), 3);
  observeStep(
    "PIPELINE",
    `Re-ranked to top ${topCandidates.length} candidates`
  );
  ...
  return {
    answer: response.content[0].text,
    sources: topCandidates.map((r) => r.title),
    hydeQuery,
    multiQueries,
  };
}
...
console.log("=".repeat(60));
console.log("M10 Lab - Step 3: Full Advanced RAG Pipeline (SOLUTION)");
console.log("=".repeat(60));
