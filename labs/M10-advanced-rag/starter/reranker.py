...
def retrieve_candidates(query, collection, bm25_index, n=10):
    """
    Broad retrieval using hybrid search.
    Returns up to n candidates (may be fewer if corpus is small).
    """
...
def print_ranked_results(label, results, scores=None):
    """Print ranked results with optional relevance scores."""
    print(f"\n--- {label} ---")
...
def rerank_with_claude(query, candidates):
    ...
    Args:
        ...
        candidates: List of document dicts with "content" and "title"

    Returns:
        List of dicts with "score" (int) and "reason" (str), one per candidate.
    """
    scores = []
    ...
    for candidate in candidates:
        ...
        #   Send a message with:
        #     - System prompt: RERANK_PROMPT
        #     - User message: f"Query: {query}\n\nPassage: {candidate['content']}"
...
def rerank(query, candidates, scores, top_k=3):
    ...
    Args:
        ...
        scores: List of score dicts from rerank_with_claude
    ...
    """
    # ------------------------------------------------------------------
    # TODO 2: Sort candidates by score descending, return top_k
    #   1. Pair each candidate with its score
    #   2. Sort by score["score"] descending
    #   3. Return the top_k candidates and their scores as two lists
    # ------------------------------------------------------------------
    return [], []
...
if __name__ == "__main__":
    print("=" * 60)
    print("M10 Lab - Step 2: Re-Ranking with Claude")
    print("=" * 60)
    ...
    # Test queries where re-ranking should improve results
    test_queries = [
