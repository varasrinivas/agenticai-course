...
class MemoryAgent {
  ...
  getSystemPrompt() {
    const wmContext = this.workingMemory.getContext();

    const procLines = ["## Available Procedural Patterns"];
    for (const [name, pattern] of Object.entries(this.proceduralMemory)) {
      procLines.push(`- **${name}**: ${pattern.description}`);
      procLines.push(`  Steps: ${pattern.steps.join(" -> ")}`);
    }
    const procContext = procLines.join("\n");

    return `You are a UCC (Uniform Commercial Code) filing research agent with a 3-tier memory system.
...
${procContext}`;
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      for (const block of response.content) {
        if (block.type === "tool_use") {
          ...
          const result = await executeTool(block.name, block.input, this);
      ...
      observeMemory("WORKING MEMORY", this.workingMemory.toDict());
    }
  ...
  async storeSessionAsEpisode(userQuery, finalResponse) {
    const debtor = this.workingMemory.get("current_debtor", "Unknown");
    const findings = this.workingMemory.get("findings_so_far", []);

    let summary;
    if (findings.length > 0) {
      const findingsText = findings.slice(0, 3).join("; ");
      summary = `Researched ${debtor} — ${findingsText}`;
    } else {
      summary = `Researched ${debtor} — ${finalResponse.substring(0, 150)}`;
    }
    ...
    const risk = this.workingMemory.get("risk_level");
    if (risk) metadata.risk_level = risk;

    const episodeId = await this.episodicMemory.storeEpisode(
      summary,
      metadata
    );
    observe("EPISODE STORED", `${episodeId}: ${summary.substring(0, 80)}...`);
    return episodeId;
  }
}
...
console.log("=".repeat(60));
console.log("M11 Lab - Step 3: Full 3-Tier Memory Agent (SOLUTION)");
console.log("=".repeat(60));
