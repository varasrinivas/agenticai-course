"""
M15 Lab — Code Interpreter Agent (Solution)
=============================================
Complete agent that writes and executes Python code in a sandbox
to analyze UCC filing data. Includes full ReAct loop, error
recovery, and trace logging.
...
def observe_thinking(text: str) -> None:
    """Print Claude's reasoning text (truncated)."""
    print(f"\n{'─' * 60}")
...
def run_code_agent(query: str, max_turns: int = 5) -> str:
    ...
    for turn in range(max_turns):
        # ── Call Claude with the conversation history and tools ──
