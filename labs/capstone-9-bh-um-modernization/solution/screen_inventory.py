...
class ViewRule:
    ...
    def validate(self) -> None:
        if self.kind not in RULE_KINDS:
            raise InventoryError(f"{self.rule!r}: kind {self.kind!r} not in {RULE_KINDS}")
        if not self.rule.strip():
            raise InventoryError("a rule needs a statement of what it says")
        if not self.source.strip():
            raise InventoryError(
                f"{self.rule!r}: quote the source. A reviewer has to be able to "
                f"find the conditional you are describing.")
        if not self.server_side_equivalent.strip():
            raise InventoryError(
                f"{self.rule!r}: state the server-side equivalent, or 'NONE'. "
                f"'NONE' is a finding and must be said out loud.")

        home = self.proposed_home.strip().lower()
        if home in REJECTED_HOMES:
            raise InventoryError(
                f"{self.rule!r}: {self.proposed_home!r} is not a relocation. "
                f"Moving a rule from JSTL to a template conditional in another "
                f"framework leaves it exactly where you found it.")
        if self.proposed_home not in HOMES:
            raise InventoryError(
                f"{self.rule!r}: proposed home {self.proposed_home!r} not in {HOMES}")
...
class ScreenInventory:
    ...
    def unreachable(self) -> list[str]:
        ...
        """
        entry = {s.route for s in self.screens if not s.reachable_from}
        if not entry:
            return [s.route for s in self.screens]

        linked = {s.route for s in self.screens
                  if any(self.by_jsp(src) for src in s.reachable_from)}
        return sorted(r for r in self.routes() if r not in entry and r not in linked)
