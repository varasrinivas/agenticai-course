...
class Check:
    ...
    def suspect(self) -> str:
        ...
        """
        if not self.expected_nonzero or self.count:
            return ""
        if self.scanned == 0:
            return ("clean, but this check scanned nothing. It did not run -- "
                    "that is not a pass.")
        if not self.could_have_fired:
            return ("clean, but the inputs cannot exercise what this check is "
                    "for, so a clean result proves nothing. " + self.note)
        return ""
...
def check_protected_content_leak(emit_root: str) -> Check:
    ...
    """
    c = Check(2, "protected-content leak", expected_nonzero=True)
    if not os.path.isdir(emit_root):
        c.note = f"nothing emitted at {emit_root}"
        return c

    files = _walk(emit_root, {".java", ".ts", ".js", ".py", ".json", ".yml",
                              ".yaml", ".sql", ".xml", ".html"})
    c.scanned = len(files)
    for path in files:
        try:
        ...
        # Line-level sinks: a log call, an event payload, an index mapping, an
        # error path -- the field and the sink appear together.
        for n, line in enumerate(lines, 1):
            if line.lstrip().startswith(("//", "#", "*", "--")):
                continue          # a comment naming the field is not a leak
            if not _NARRATIVE_FIELD.search(line):
                continue
            for sink, pattern in _SINKS:
                if pattern.search(line):
                    c.findings.append(Finding(
                        where=_rel(emit_root, path), line=n,
                        detail=f"{sink}: {line.strip()[:140]}"))
                    break

        # Statement-level sink: a narrative COLUMN inside an audit table.
        #
        # A line-by-line scan misses this, because `old_narrative TEXT,` and
        # `CREATE TABLE bh_audit_event` are on different lines. The legacy
        # system's audit trigger copies the full narrative on every update, so
        # an authorization touched twelve times leaves twelve copies -- in a
        # table with no consent scope and no expiry. Reproducing that is a
        # finding worth its own detection.
        if path.endswith(".sql"):
            body = "".join(lines)
            for stmt in re.findall(r"CREATE TABLE[^;]*;", body, re.I | re.S):
                if not re.search(r"CREATE TABLE\s+\w*audit", stmt, re.I):
                    continue
                cols = [ln.strip() for ln in stmt.splitlines()
                        if _NARRATIVE_FIELD.search(ln)
                        and not ln.lstrip().startswith("--")]
                for col in cols:
                    c.findings.append(Finding(
                        where=_rel(emit_root, path),
                        detail=f"audit: narrative column in an audit table "
                               f"({col[:100]}). No consent scope, no expiry, one "
                               f"copy per update."))
    return c
...
def check_consent_atomicity(emit_root: str, seam_map: dict | None = None) -> Check:
    ...
    yet.
    """
    c = Check(4, "consent atomicity", expected_nonzero=True)

    sql_files = _walk(emit_root, {".sql"}) if os.path.isdir(emit_root) else []
    c.scanned = len(sql_files)
    sql = " ".join(_read(p) for p in sql_files)

    has_consent_table = bool(re.search(r"CREATE TABLE\s+\w*consent", sql, re.I))
    if not has_consent_table:
        c.findings.append(Finding(
            where="migrations",
            detail="no consent table in the emitted schema. 42 CFR Part 2 "
                   "disclosure requires a consent that names the recipient; "
                   "there is nowhere to record one."))
        return c

    # Is the invariant enforced, or merely currently true?
    enforced = bool(re.search(
        r"(REFERENCES\s+\w*consent|consent_id\s+\w+\s+NOT NULL|"
        r"CHECK\s*\(.*consent)", sql, re.I))
    if not enforced:
        c.findings.append(Finding(
            where="migrations",
            detail="a consent table exists but nothing enforces that an "
                   "authorization has one -- no foreign key, no NOT NULL "
                   "consent_id, no constraint. The state is clean today and "
                   "reachable tomorrow."))

    if seam_map:
        for seam in seam_map.get("seams", []):
            if seam.get("rejected_because"):
                continue
            crosses = " ".join(seam.get("crosses", []))
            if "submitAndDecide" in crosses and not seam.get("replacement"):
                c.findings.append(Finding(
                    where=f"seam {seam.get('name')}",
                    detail="the submit transaction crosses this seam with no "
                           "stated replacement for its atomicity. Compensation "
                           "is not available for a disclosure: you cannot "
                           "un-hold protected content you have already held."))
    return c
...
def check_screen_coverage(emit_root: str, inventory: dict | None = None) -> Check:
    ...
    determination.
    """
    c = Check(8, "screen coverage")
    if not os.path.isdir(emit_root):
        c.note = f"nothing emitted at {emit_root}"
        return c

    client = _walk(emit_root, {".ts", ".html"})
    c.scanned = len(client)
    if not client:
        c.note = "no client emitted -- phase 9B did not run"
        return c

    blob = " ".join(_read(p) for p in client)

    # Routes must exist at all. The reference platform declares the router as a
    # dependency and never calls provideRouter, so there is no example to copy.
    if "provideRouter" not in blob and "Routes" not in blob:
        c.findings.append(Finding(
            where="apps/bh-intake-ui",
            detail="no router wired. The donor declares @angular/router as a "
                   "dependency and provides only provideHttpClient(), so copying "
                   "its bootstrap gives a single unrouted form."))

    # Every screen in the inventory reaches a route.
    #
    # Matched as a route DECLARATION rather than as a bare substring. "member"
    # appears in `memberLastName` in half a dozen components, so a substring
    # test reports every screen as routed whatever the routes file says.
    declared = set(re.findall(r"path:\s*['\"]([^'\"]*)['\"]", blob))
    declared_first = {p.split("/")[0] for p in declared if p}
    for screen in (inventory or {}).get("screens", []):
        route = screen.get("route", "")
        path = route.lstrip("/").split("/")[0]
        if path and path not in declared_first:
            c.findings.append(Finding(
                where=screen.get("jsp", "?"),
                detail=f"no route declared for {route} -- this screen has "
                       f"disappeared. The code compiles and a file count passes."))

    # Every rule extracted from a view must have landed somewhere real.
    for screen in (inventory or {}).get("screens", []):
        for rule in screen.get("rules", []):
            home = str(rule.get("proposed_home", "")).lower()
            if home in {"template-conditional", "ngif", "*ngif", "client-side"}:
                c.findings.append(Finding(
                    where=screen.get("jsp", "?"),
                    detail=f"{rule.get('rule')!r} was relocated to "
                           f"{rule.get('proposed_home')!r}, which is where it was "
                           f"found."))

    # A role compared as a NUMBER is the legacy approximation carried across.
    for path in client:
        text = _read(path)
        for m in re.finditer(r"roleMask\s*(>=|>|<=|<)\s*\d+", text):
            c.findings.append(Finding(
                where=_rel(emit_root, path),
                detail=f"numeric role comparison `{m.group(0)}` carried over from "
                       f"JSTL. The view was the permissive side of that "
                       f"divergence; test named roles instead."))

    # The shared components exist to be used.
    if "libs/ui" in blob or "@bh-um-lite/ui" in blob:
        pass
    else:
        c.findings.append(Finding(
            where="apps/bh-intake-ui",
            detail="the shared UI library is not imported anywhere. The donor "
                   "exports three components and imports none of them; "
                   "reproducing that is how a shared library dies."))

    # Configuration, not a hardcoded laptop.
    #
    # Comment lines are skipped: the generated environment file WARNS about the
    # donor's hardcoded URL by quoting it, and flagging that would mean the only
    # way to pass is to stop explaining the mistake.
    for path in client:
        for n, line in enumerate(_read(path).splitlines(), 1):
            stripped = line.lstrip()
            if stripped.startswith(("//", "*", "/*", "#")):
                continue
            m = re.search(r"https?://(localhost|127\.0\.0\.1)[:/][^\s\"']*", line)
            if m:
                c.findings.append(Finding(
                    where=_rel(emit_root, path), line=n,
                    detail=f"hardcoded service URL {m.group(0)!r}. The donor "
                           f"embeds one in a component, which makes the client "
                           f"unshippable to any environment but a laptop."))
    return c
