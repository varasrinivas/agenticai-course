...
def looks_like_protected_content(text: str) -> bool:
    ...
    if not text or len(text) < 60:
        return False
    if _NARRATIVE_FIELDS.search(text) and _PROSE.search(text):
        return True
    if _CLINICAL_REGISTER.search(text) and _PROSE.search(text):
        return True
    return False
...
def redact_narrative(text: str) -> str:
    ...
    def _sub(m: re.Match) -> str:
        return f"[PROTECTED-CONTENT-REDACTED: {len(m.group(0))} chars]"

    return _PROSE.sub(_sub, text)
...
def filter_tool_result(tool_name: str, tool_input: dict, result_text: str) -> tuple[str, bool]:
    ...
    appears.
    """
    if not result_text or not looks_like_protected_content(result_text):
        return result_text, False

    path = _source_path_of(tool_input)

    if is_allowlisted(path):
        # Synthetic fixture. Allowed through, but budgeted -- an agent that
        # reads the whole seed file accumulates a clinical record in its
        # transcript one tool call at a time.
        trimmed = excerpt(result_text)
        return trimmed, trimmed != result_text

    return (
        redact_narrative(result_text)
        + "\n[protected-content gate: narrative-shaped runs redacted; source "
          f"{path or 'unknown'} is not on the synthetic-fixture allowlist]",
        True,
    )
...
async def enforce_source_readonly(tool_name: str, tool_input: dict, context: Any):
    ...
    root = None
    for prefix, r in _READONLY_PREFIXES.items():
    ...
    for key in ("path", "subpath", "file", "source"):
        value = tool_input.get(key)
        if isinstance(value, str) and value and _escapes(root, value):
            return PermissionResultDeny(
                message=(
                    f"Path {value!r} resolves outside {os.path.basename(root)}/. "
                    f"Both source trees are read-only and self-contained: they are "
                    f"EVIDENCE. A parity validator that diffs against a tree the "
                    f"agent can reach outside of is diffing against a moving target."
                )
            )
    return PermissionResultAllow()
...
async def hitl_finalization_gate(tool_name: str, tool_input: dict, context: Any):
    ...
    """
    if not tool_name.endswith("finalize_modernization"):
        return PermissionResultAllow()

    if config.FINALIZATION_APPROVED:
        return PermissionResultAllow()

    return PermissionResultDeny(
        message=(
            "FINALIZATION REQUIRES HUMAN APPROVAL.\n\n"
            + finalization_briefing()
            + "\n\n"
            "A person must read artifacts/modernization_report.html and re-run:\n"
            "    python coordinator.py --phase finalize --approve\n\n"
            "This is not an obstacle to work around. This modernization changes how "
            "medical-necessity determinations are made; the agent does not get to "
            "decide that its own work is ready. Report the state above to the "
            "operator and stop."
        )
    )
...
def redact(text: str) -> str:
    ...
    """
    for pattern in _SECRET_PATTERNS + _ACTOR_PATTERNS:
        if pattern.groups >= 3:
            text = pattern.sub(r"\1***\3", text)
        else:
            text = pattern.sub(r"\1***", text)
    return redact_narrative(text)
