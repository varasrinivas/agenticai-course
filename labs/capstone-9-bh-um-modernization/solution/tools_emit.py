...
async def record_gap(args: dict) -> dict:
    ...
    entry = GapEntry(
        ...
        harm=str(args.get("harm") or "").strip(),
        requirement=str(args.get("requirement") or "").strip(),
        trap_id=int(args["trap_id"]) if args.get("trap_id") else None,
        backlog=str(args.get("backlog") or "").strip(),
    )
    if not entry.capability:
        return _err("capability is required")

    try:
        register.add(entry)
    except RegisterError as exc:
        # Returned as an error rather than accepted-with-a-warning. A register
        # that accepts a must-not-port with no harm is a register whose most
        # important verdict means nothing.
        return _err(str(exc), capability=entry.capability, verdict=entry.verdict)

    register.save(_REGISTER_PATH)
    return _ok({
        "recorded": entry.capability,
...
async def queue_manual_review(args: dict) -> dict:
    ...
    if not artifact or not reason:
        return _err("artifact and reason are both required")

    question = str(args.get("question") or "").strip()
    if not question:
        return _err(
            "question is required. Queueing something without stating what a "
            "human has to decide produces a list nobody can act on -- name the "
            "specific question, e.g. 'which determinations was LEGACY_OVERRIDE "
            "meant to cover, and who authorised it?'",
            artifact=artifact,
        )
    ...
    return _ok({"queued": artifact, "queue_length": len(items)})
...
async def eval_rules(args: dict) -> dict:
    ...
    if engine not in {"legacy", "dmn"}:
        return _err("engine must be 'legacy' or 'dmn'")

    raw = args.get("case_json")
    try:
        payload = json.loads(raw) if isinstance(raw, str) else dict(raw or {})
    except (json.JSONDecodeError, TypeError, ValueError) as exc:
        return _err(f"case_json is not valid JSON: {exc}")

    try:
        case = Case.from_dict(payload)
    except (TypeError, ValueError) as exc:
        return _err(f"could not read the case: {exc}")

    if engine == "legacy":
        d = evaluate_legacy(case)
        return _ok({"engine": "legacy", "auth_id": case.auth_id, "decision": d.to_dict()})

    if not os.path.exists(_IR_PATH):
        return _err(f"no rules IR at {_IR_PATH} -- run the extract-rules phase first")
    try:
        with open(_IR_PATH, encoding="utf-8") as fh:
            ir = json.load(fh)
    except (OSError, json.JSONDecodeError) as exc:
        return _err(f"could not read the rules IR: {exc}")

    try:
        d = evaluate_ir(ir, case)
    except HitPolicyError as exc:
        # Not a crash to route around. The table is reporting that the ladder's
        # ordering carried information the table does not.
        return _err(f"hit policy: {exc}", auth_id=case.auth_id,
                    hit_policy=ir.get("hit_policy"))
    return _ok({"engine": "dmn", "auth_id": case.auth_id, "decision": d.to_dict()})
