 ...
 *
 * YOUR TASK: Fill in the TODO sections to complete the agentic loop.
 *
 * Usage:
 *     node agent_loop.js
 */
...
class SimulatedAgentSDK {
  ...
  query(prompt, system = null) {
    ...
    while (this.turnCount < this.maxTurns) {
      ...
      // TODO: Check response.stopReason and handle each case:
      //
      // Case 1: "end_turn"
      //   - console.log that the task is complete
      //   - console.log the first 100 chars of response.content
      //   - Call this._log("end_turn", "Task completed naturally")
      //   - Return the response
      //
      // Case 2: "tool_use"
      //   - Loop through response.toolCalls
      //   - For each tool call, extract name and input
      //   - console.log the tool call details
      //   - Execute: const result = executeTool(toolName, toolInput)
      //   - console.log the result (first 80 chars)
      //   - Call this._log("tool_use", `${toolName}: ${result.slice(0, 50)}`)
      //   - Push assistant message with tool_use content block to this.messages
      //   - Push user message with tool_result content block to this.messages
      //
      // Case 3: "max_tokens"
      //   - console.log max tokens warning
      //   - Call this._log("max_tokens", "Token limit hit")
      //   - Return the response
      //
      // IMPORTANT: Do NOT parse response.content to determine if done!

      // Replace the line below with your implementation
      continue;
    }

    // TODO: Handle the case where maxTurns is exceeded
    // - console.log a warning that maxTurns was reached
    // - Note that this is a safety net, not the primary stop mechanism
    // - Call this._log("max_turns_exceeded", `Hit ${this.maxTurns} turn limit`)
    // - Return a new AgentResponse with stopReason "max_turns"

    // Replace the line below with your implementation
    return new AgentResponse("max_turns", "Not implemented yet", []);
  }
