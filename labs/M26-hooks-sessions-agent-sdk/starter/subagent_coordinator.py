...
class Coordinator:
    ...
    def process_request(self, request):
        ...
        # TODO: Step 2 — Execute subtasks
        #
        # Create an empty results dict.
        # For each subtask:
        #   1. Get the role from the subtask
        #   2. Check if a subagent exists for this role (in self.subagents)
        #      - If not, print a warning and continue
        #   3. Get the subagent
        #   4. Call subagent.execute(subtask["description"], subtask.get("context", {}))
        #   5. Store the result in results[role]
        #   6. Append to self.execution_log: {timestamp, subagent name, role, task (first 60 chars), execution_time_ms}
        #
        #   IMPORTANT: After entity_resolution completes, enrich the risk_scoring subtask:
        #   - Extract entity names from the entity_resolution result
        #   - entities = [result["canonical_entity"].get("canonical_name", "")]
        #   - Add related entity names to the list
        #   - Update the risk_scoring subtask's context["entities"] with this enriched list
        #   This demonstrates how the coordinator passes information between subagents.

        results = {}  # <-- Your implementation goes after this line
    ...
    def aggregate_results(self, results):
        ...
        #
        # Create a report dict with three top-level keys:
        #
        # "summary": {
        ...
        #
        # "provenance": {
        #     "filing_data": {"source": "filing_search subagent", "filings": filing_results.get("filings", [])},
        #     "entity_data": {"source": "entity_resolution subagent", "canonical": entity_results.get("canonical_entity", {}), "related": entity_results.get("related_entities", [])},
        #     "risk_data": {"source": "risk_scoring subagent", "profiles": risk_results.get("individual_profiles", [])}
        # }

        report = {}  # <-- Replace this with your implementation
...
def main():
    ...
    print(f"\n  Request type: {summary.get('request_type', 'N/A')}")
    print(f"  Subagents used: {', '.join(summary.get('subagents_used', []))}")

    print(f"\n  Findings:")
    print(f"    Total filings found: {findings.get('total_filings', 'N/A')}")
    print(f"    States covered: {', '.join(findings.get('states_covered', []))}")
    print(f"    Related entities: {findings.get('total_related_entities', 'N/A')}")
    confidence = findings.get('resolution_confidence', 0)
    print(f"    Entity resolution confidence: {confidence:.0%}" if isinstance(confidence, (int, float)) else f"    Entity resolution confidence: N/A")
    print(f"    Aggregate risk level: {findings.get('aggregate_risk_level', 'N/A')}")
    print(f"    Aggregate risk score: {findings.get('aggregate_risk_score', 'N/A')}")
    lien = findings.get('total_lien_exposure', 0)
    print(f"    Total lien exposure: ${lien:,}" if isinstance(lien, (int, float)) else f"    Total lien exposure: N/A")

    print(f"\n  Provenance:")
    for source_name, source_data in report.get("provenance", {}).items():
        print(f"    {source_name}: from {source_data.get('source', 'N/A')}")
