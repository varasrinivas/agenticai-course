...
class HookEngine:
    ...
    def execute_with_hooks(self, tool_name, tool_input, tool_executor):
        ...
        #   - Call self.run_pre_hooks(tool_name, tool_input)
        #   - If not allowed, print the reason and return {"blocked": True, "reason": reason}
        #   - If allowed, print "[OK] PreToolUse hooks passed"
        #
        # Step 2: Execute the tool
        #   - Call tool_executor(tool_name, tool_input) to get the result
        #   - Print the result (first 80 chars)
        #
        # Step 3: Run post-hooks
        #   - Call self.run_post_hooks(tool_name, tool_input, result)
        #   - Print "[OK] PostToolUse hooks completed"
        #
...
def refund_limit_hook(tool_name, tool_input):
    ...
    #
    # 1. If tool_name is not "issue_refund", return {"allowed": True}
    # 2. Get the "amount" from tool_input (default to 0)
    # 3. If amount > 500, return:
    #    {"allowed": False, "reason": f"Refund amount ${amount:.2f} exceeds $500 limit. Requires human approval. Escalate to support-team-lead."}
...
def production_write_guard(tool_name, tool_input):
    ...
    #
    # 1. If tool_name is not "Write", return {"allowed": True}
    # 2. Get the "file_path" from tool_input (default to "")
    # 3. If "data/production" is in file_path, return:
    #    {"allowed": False, "reason": f"Cannot write to production data path: {file_path}"}
    # 4. Otherwise return {"allowed": True}
...
def audit_log_hook(tool_name, tool_input, tool_output):
    ...
    #
    # 1. Create a log_entry dict with:
    #    - timestamp: datetime.now().isoformat()
    #    - tool: tool_name
    #    - input_summary: first 100 chars of json.dumps(tool_input)
    #    - output_summary: first 100 chars of json.dumps(tool_output)
    #    - status: "blocked" if tool_output has a "blocked" key, else "success"
    # 2. Print: f"     [Audit] {tool_name} -> {log_entry['status']}"
