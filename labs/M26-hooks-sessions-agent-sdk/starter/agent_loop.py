...
class SimulatedAgentSDK:
    ...
    def query(self, prompt, system=None):
        ...
        while self.turn_count < self.max_turns:
            ...
            # TODO: Check response.stop_reason and handle each case:
            #
            # Case 1: "end_turn"
            #   - Print that the task is complete
            #   - Print the first 100 chars of response.content
            #   - Call self._log("end_turn", "Task completed naturally")
            #   - Return the response
            #
            # Case 2: "tool_use"
            #   - Loop through response.tool_calls
            #   - For each tool call, extract name and input
            #   - Print the tool call details
            #   - Execute: result = MockToolResult.execute(tool_name, tool_input)
            #   - Print the result (first 80 chars)
            #   - Call self._log("tool_use", f"{tool_name}: {result[:50]}")
            #   - Append assistant message with tool_use content block to self.messages
            #   - Append user message with tool_result content block to self.messages
            #
            # Case 3: "max_tokens"
            #   - Print max tokens warning
            #   - Call self._log("max_tokens", "Token limit hit")
            #   - Return the response
            #
            # IMPORTANT: Do NOT parse response.content text to determine if done!
            # stop_reason is the ONLY reliable termination signal.

            pass  # <-- Replace this with your implementation

        # TODO: Handle the case where max_turns is exceeded
        # - Print a warning that max_turns was reached
        # - Note that this is a safety net, not the primary stop mechanism
        # - Call self._log("max_turns_exceeded", f"Hit {self.max_turns} turn limit")
        # - Return an AgentResponse with stop_reason="max_turns"
