...
class SupportAgent:
    ...
    def __init__(self):
        ...
        # Register hooks
        self.hooks.register_pre_hook(
            "issue_refund",
            self._refund_limit_hook,
            "Block refunds > $500"
        )
        self.hooks.register_post_hook(
            "*",
            self._audit_hook,
            "Audit trail"
        )
    ...
    def _handle_filing_lookup(self, session_name, request_text):
        ...
        result = self.hooks.execute_with_hooks(
            "lookup_filing",
        ...
        # Turn 2: Check risk
        risk = self.hooks.execute_with_hooks(
            "check_risk_profile",
            {"entity_name": result.get("debtor", "Unknown")},
            self._execute_tool
        )
        print(f"    Turn 2: check_risk_profile -> {risk.get('risk_level', 'N/A')}")

        # Turn 3: Deliver answer (end_turn)
        response = (
            f"Filing {result['filing_number']} is {result['status']}.\n"
            f"Debtor: {result.get('debtor', 'N/A')}\n"
            f"Risk: {risk.get('risk_level', 'N/A')} (score: {risk.get('risk_score', 'N/A')})\n"
    ...
    def _handle_refund(self, session_name, request_text):
        ...
        # Attempt refund (hooks will check limits)
        result = self.hooks.execute_with_hooks(
            "issue_refund",
            {"amount": amount, "reason": "Customer request"},
            self._execute_tool
        )

        if result.get("blocked"):
            print(f"    Refund BLOCKED: {result['reason']}")

            # Escalate — this is a POLICY GAP (amount too high), not sentiment
            escalation = self.hooks.execute_with_hooks(
                "escalate_to_human",
                {
                    "priority": "high",
                    "reason": f"Refund of ${amount:.2f} exceeds automated limit",
                    "context": f"Customer requested refund. Amount exceeds $500 hook limit."
                },
                self._execute_tool
            )

            response = (
                f"I apologize, but I cannot process a refund of ${amount:.2f} automatically.\n"
                f"Refunds over $500 require human approval.\n"
                f"I have escalated your request (ticket: {escalation.get('ticket_id', 'N/A')}).\n"
                f"Expected response time: {escalation.get('eta_minutes', 'N/A')} minutes."
            )

            self.sessions.add_message(session_name, "assistant", response)
            print(f"\n  Response: {response}")
            return {"status": "escalated", "response": response, "ticket_id": escalation.get("ticket_id")}
        else:
            response = (
                f"Refund of ${amount:.2f} has been processed.\n"
    ...
    def _handle_entity_research(self, session_name, request_text):
        ...
        # Fork the session for subagent exploration
        fork_name = f"{session_name}-research-fork"
        self.sessions.fork_session(session_name, fork_name)
        print(f"    Forked session: {fork_name}")

        # Delegate to coordinator
        research = self.coordinator.delegate_entity_research(entity_name)

        # Build response from aggregated results
        response = (
            f"Cross-state research complete for {entity_name}:\n"
            f"- Entities found: {len(research['entities'])} ({', '.join(research['entities'])})\n"
            f"- States covered: {', '.join(research['states'])}\n"
            f"- Total filings: {len(research['filings'])}\n"
            f"- Aggregate risk: {research['aggregate_level']} (score: {research['aggregate_risk']})\n"
        )

        for rp in research["risk_profiles"]:
            response += f"  - {rp['entity']}: {rp['risk']['risk_level']} ({rp['risk']['risk_score']})\n"
