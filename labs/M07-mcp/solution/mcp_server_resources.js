 ...
 *     node M07-mcp/solution/mcp_server_resources.js
 */
...
server.resource(
  ...
  "ucc://filings",
  async (uri) => {
    const lines = ALL_FILINGS.map(
      (f) =>
        `${f.filing_number} | ${f.debtor.name || "(missing)"} | ${f.state} | ${f.status}`
    );
    const text = `UCC Filings Index (${ALL_FILINGS.length} total):\n\n${lines.join("\n")}`;
...
server.resource(
  "filing_detail",
  new ResourceTemplate("ucc://filing/{filing_number}", { list: undefined }),
  async (uri, { filing_number }) => {
    const filing = getFilingByNumber(filing_number);
    if (!filing) {
      return { contents: [{ uri: uri.href, text: `No filing found: ${filing_number}` }] };
    }
    return { contents: [{ uri: uri.href, text: JSON.stringify(filing, null, 2) }] };
  }
);
