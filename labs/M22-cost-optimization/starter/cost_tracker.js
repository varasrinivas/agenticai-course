...
function selfTest() {
  ...
  assert.ok(total > 0, "FAIL: Total cost should be > 0");
  console.log("  PASS: Total cost is positive");

  console.log(`  Models used: ${Object.keys(byModel).join(", ")}`);
  assert.ok(Object.keys(byModel).length >= 2, "FAIL: Should have multiple models");
  ...
  assert.ok(cacheInfo.cacheHits > 0, "FAIL: Should have cache hits");
  ...
  assert.ok(routingInfo.savings > 0, "FAIL: Routing should save vs all-Sonnet");
