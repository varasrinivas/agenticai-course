"""
Tests for Capstone 2, Domain B (Ecommerce) — Loader and Chunker
...
class TestLoadDocuments:
    ...
    def test_loads_all_five_docs(self):
        """load_documents() should find all 5 ecommerce documents in docs/."""
    ...
    def test_filenames_are_expected(self):
        """The loaded filenames should match the known ecommerce docs."""
        expected_files = sorted([
            "contract_acme_corp.md",
            "contract_globex_industries.md",
            "contract_initech_solutions.md",
            "product_catalog.md",
            "shipping_policies.md",
        ])
        docs = load_documents(DOCS_DIR)
...
class TestChunkDocument:
    ...
    def test_produces_chunks_for_real_doc(self):
        """Chunking a real ecommerce doc should produce at least 1 chunk."""
        docs = load_documents(DOCS_DIR)
    ...
    def test_chunks_have_correct_metadata(self):
        ...
        doc = {"filename": "product_catalog.md", "content": "X" * 500}
        chunks = chunk_document(doc, chunk_size=200, overlap=50)
        for i, chunk in enumerate(chunks):
            assert chunk["source"] == "product_catalog.md", (
